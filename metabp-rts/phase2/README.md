...
phase2/
├── config/
│   └── phase2_config.yaml       ← paramètres centraux (mode BP, seuils, sorties)
├── bp/
│   ├── graph_inverter.py        ← Étape 1 : G → DG (arcs inversés)
│   ├── bp_initializer.py        ← Étape 2 : vecteur p0 enrichi Phase 1
│   ├── bp_propagator.py         ← Étape 3 : propagation itérative Noisy-OR / Max-Product
│   └── cit_builder.py           ← Étapes 4-5 : CIT + Θ_complet + S_écho-impact
├── selection/
│   └── test_scorer.py           ← Étape 5 : tiering + scoring + T_sel
├── scripts/
│   └── run_phase2.py            ← orchestrateur principal
└── data/
