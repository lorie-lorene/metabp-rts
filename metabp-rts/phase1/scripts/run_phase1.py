...
def run_pipeline(config: dict, services_map: dict, base_dir: Path) -> None:
    ...
    with open(t_path, "w", encoding="utf-8") as f:
        json.dump(
           [{"trace_id": tp.trace_id,
              ...
              "http_method": tp.http_method,
              "entry_service": tp.entry_service}
    ...
    # -- Generation des MR data-driven (Latence / Erreur / Completude) --
    # Regenerees a CHAQUE execution depuis les spans deja parses (etape 2),
    # comme le graphe et les seuils : le catalogue MR est un artefact derive
    # des donnees, jamais statique. Les champs threshold_* sont preserves
    # pour le verificateur et le mutateur de la Phase 4.
    import yaml as _yaml
    from mr_catalog.mr_generator_full import MRGeneratorFull

    _dd_gen = MRGeneratorFull(percentile=99.0, error_margin=0.10)
    _dd_mrs = _dd_gen.generate_from_spans(spans)

    _cat_path = resolve(paths["mr_catalog"])
    with open(_cat_path, encoding="utf-8") as _f:
        _cat = _yaml.safe_load(_f) or {}
    _key = "mr_catalog" if "mr_catalog" in _cat else "relations"
    _cat.setdefault(_key, [])
    _existing_ids = {m.get("id") for m in _cat[_key]}
    _added = [m for m in _dd_mrs if m["id"] not in _existing_ids]
    _cat[_key].extend(_added)
    with open(_cat_path, "w", encoding="utf-8") as _f:
        _yaml.dump(_cat, _f, default_flow_style=False,
                   allow_unicode=True, sort_keys=False)

    _nl = sum(1 for m in _added if m.get("type") == "Latence")
    _ne = sum(1 for m in _added if m.get("type") == "Erreur")
    _nc = sum(1 for m in _added if m.get("type") == "Completude")
    logger.info("MR data-driven ajoutees : %d (%d Latence, %d Erreur, %d Completude)",
                len(_added), _nl, _ne, _nc)
