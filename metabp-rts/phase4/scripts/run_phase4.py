...
run_phase4.py
=============
Orchestrateur Phase 4 MetaBP-RTS — Validation sans oracle.
...
Options :
    --skip-4a   : sauter la sous-phase 4A (mutation)
    --skip-4b   : sauter la sous-phase 4B (vérification MR)
"""
...
def load_mr_catalog_for_mutation(mr_catalog_path: str) -> dict:
    """
    Charge le catalogue MR et retourne { service: [mr_type, ...] }
    pour la table de détection du MutationVerifier.
    """
    with open(mr_catalog_path, encoding="utf-8") as f:
    ...
    service_to_types = defaultdict(list)
    for mr in relations:
        svc     = mr.get("service", mr.get("service_name", ""))
        mr_type = mr.get("type", "")
        if svc and mr_type:
            service_to_types[svc].append(mr_type)

    return dict(service_to_types)
...
def compute_metrics(
    ...
    """
    Calcule les métriques finales EN, FCS, R_proxy, P_proxy, F.

    EN   = (N_original - N_sel) / N_original × 100
    FCS  = mutants_tués / mutants_totaux × 100  (proxy du Recall)
    P_mr = MR_pass / MR_total × 100              (proxy de la Précision)
    F    = 2 × FCS × P_mr / (FCS + P_mr)
    """
    ...
    return {
        ...
        "P_mr":  p_mr,
        "F":     f,
        "note":  "FCS = proxy du Recall (R) en boîte noire via mutation de traces",
    }
...
def main():
    ...
    parser.add_argument(
        "--config", default="../config/phase4_config.yaml",
    )
    parser.add_argument(
        "--skip-4a", action="store_true",
        help="Sauter la sous-phase 4A (mutation de traces)",
    )
    parser.add_argument(
        "--skip-4b", action="store_true",
        help="Sauter la sous-phase 4B (vérification MR)",
    )
    ...
    if not args.skip_4a:
        ...
        # Vérification des mutants
        mr_for_mutation = load_mr_catalog_for_mutation(mr_catalog_path)
        verifier_4a = MutationVerifier(mr_catalog=mr_for_mutation)
        ...
        logger.info(
            "4A terminée : %d mutants | %d tués | FCS=%.1f%%",
            mutation_result["n_total"],
            mutation_result["n_killed"],
            mutation_result["fcs"],
        )
    else:
    ...
    if not args.skip_4b:
        ...
        logger.info(
            "4B terminée : %d MR vérifiées | %d PASS | %d FAIL | taux=%.1f%%",
    ...
    if mutation_result:
        logger.info("  FCS (qualité)   : %.1f%%  (%d/%d mutants tués)",
                    ...
                    mutation_result["n_total"])
    if mr_result:
        logger.info("  P_mr (précision): %.1f%%  (%d/%d MR passées)",
                    metrics["P_mr"],
                    mr_result["n_pass"],
                    mr_result["n_mr_verified"])
        logger.info("  Violations MR   : %d  (régressions potentielles)",
                    mr_result["n_fail"])
    logger.info("  F-measure       : %.1f%%", metrics["F"])
    logger.info("=======================================================")

    if mr_result and mr_result["n_fail"] == 0:
        logger.info("CONCLUSION : Aucune régression détectée après ΔS")
    elif mr_result and mr_result["n_fail"] > 0:
        logger.warning(
            "CONCLUSION : %d régression(s) potentielle(s) détectée(s)",
            mr_result["n_fail"],
        )
